...
Dunietz et al. unterscheiden (wie Talmy) zwischen fördernden und hemmenden Relationen. Schließlich überführen Wu et al. die extrahierten Attributionen in Subjekt-Prädikat-Objekt Tripel, die zu Causal-Knowledge Graphs aggregiert werden.

In der Summe finden sich damit eine große Zahl bestehender Ansätze, die im Folgenden zur Kausalsemantik zusammengeführt und durch Prinzipien der Polykausalität und propositionalen Negation erweitert werden.
:::
...
Aus Gründen der Vergleichbarkeit wurden verbatime Zeichenketten (bspw. Ernte, Erdbeer-Bauern in der rechten Tabelle) zu insgesamt 12 Kategorien (bspw. Landwirtschaft) aggregiert, von denen sieben in der linken Tabelle zu sehen sind.
:::
...
::: {.notes}
Im Falle Bienensterben wird dabei eine drastische Verschiebung der Wirkungen ersichtlich. Zunächst liegt der (destruktive) Fokus auf bedrohten Imkern und Ernteausfällen durch Bestäubungsverluste (linke Seite). 
...
Im Falle Artensterben verschieben sich Interventen von generisch-menschlichen Einheiten zunehmend in Richtung der Kategorie ABSTRAKT – vor allem in Form von Pronomina wie 'jemand' oder 'etwas tun'.
:::
...
::: {.notes}
Jenseits der diachronen Detailanalyse dienen die annotierten WABI-Sätze auch als Daten zum Training eines eigens entwickelten Multi-Task Transformers, der die sprachlichen Muster mittels modifizierter, numerischer Gewichtsmatrizen verinnerlicht.
...
Neben der Visualisierung lässt sich der ACG – beispielsweise eingebettet in die Graphdatenbank Neo4j – nach Beliebenm abfragen, um unter anderem unmittelbare und einfach nachgelagerte Wirkungen von Klimawandel ausgeben.
:::
...
- $K_U$ dominiert durch *Mensch*, *Klimawandel*, [Emission]{.smallcaps}
