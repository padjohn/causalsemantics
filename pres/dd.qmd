...
In der Summe finden liegen eine große Zahl bestehender Ansätze zur Extraktion und – zu einem geringeren Teil – der Projektion vor. Beide Stränge werden im Folgenden zur Kausalsemantik verwoben, die darüber hinaus noch den Prinzipien der Polykausalität Rechnung trägt.
:::
...
In einem zweistufigen Verfahren klassifiziert C-BERT sowohl Spannen (Token, oben) als auch Relationen (unten). Letztere werden als Ergebnis maschineller 'Annotation' ebenso in (Cause, Effect, Influence) Tupeln – hier: (Bundesregierung, Waldsterben, -1) – überführt.  
:::
...
MATCH (c:Klimawandel)-[i:influence]->(e)
RETURN c, e, i
```
...
::: {.notes}
Auf die 22 Millionen Sätze des Umweltkorpus angewandt, ergibt sich ein Attributionaler Kausalgraph (ACG) bestehend aus 360.000 Entitäten und 1.6 Millionen Relationen, von denen hier 212 Entitäten und 690 Relationen abgebildet sind.  

Neben der Visualisierung lässt sich der ACG – beispielsweise eingebettet in die Graphdatenbank Neo4j – nach Belieben abfragen, um unmittelbare (und wahlweise auch mittelbare) Wirkungen von Klimawandel (s. unten rechts) auszugeben.
:::
