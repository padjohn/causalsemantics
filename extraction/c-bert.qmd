...
C-BERT [@cbert] is a multi-task transformer to extract fine-grained causal relations as $(C, E, I)$ tuples from text. It classifies **tokens** as entities and indicators using BIOES [@ramshaw-marcus-1995-text] tags, then classifies their **relationships**. 

<br>

Each indicator → entity relation is described in terms of **role**, **polarity** and **salience** – either by a unified attention-head or three separate heads.
...
The factorization is linguistically motivated: an entity's **role** depends on its syntactic position, its **polarity** on indicator class and negation, and its **salience** on determiners, coordination, and context markers. Each head can tend on its own type of signal.

The German [models](https://huggingface.co/collections/pdjohn/causalbert) are built on the [EuroBERT](https://huggingface.co/collections/EuroBERT/eurobert) family [@boizard2025eurobertscalingmultilingualencoders], fine-tuned via LoRA [@hu2021loralowrankadaptationlarge], using newspaper articles and [plenary debates](https://huggingface.co/datasets/pdjohn/bundestag-causal-attribution) focused on biodiversity topics (see [Annotation](annotation.qmd)).
...
- For a high-level view of the extraction pipeline, see [Extraction Overview](extraction.qmd.qmd)
- For the theoretical framework motivating polarity and salience, see [Framework](../framework/framework.qmd)
