...
This page presents the annotation schema and guidelines used to produce the training data for [C-BERT](c-bert.qmd) and the underlying $(C, E, I)$ tuple representation. 
...
| ----------------------------------------------------------------- | ----- | --------- | --- | ------------------------------------------------------- | ----- | --------- |
| [Ursache]{.smallcaps} <br> '[Cause]{.smallcaps}'                  | 21    | 167       |     | [Beitrag]{.smallcaps} <br> '[Contribution]{.smallcaps}' | 10    | 70        |
| [Verantwortung]{.smallcaps}  <br>  '[Responsibility]{.smallcaps}' | 11    | 122       |     | [Folge]{.smallcaps} <br> '[Consequence]{.smallcaps}'    | 6     | 69        |
| [Stoppen]{.smallcaps}  <br>  '[Stop]{.smallcaps}'                 | 3     | 111       |     | [Kampf]{.smallcaps} <br> '[Fight]{.smallcaps}'          | 8     | 69        |
| [Gegen]{.smallcaps}  <br>  '[Against]{.smallcaps}'                | 3     | 95        |     | [Führen]{.smallcaps} <br> '[Lead]{.smallcaps}'          | 9     | 59        |
| [Durch]{.smallcaps}  <br> '[through]{.smallcaps}'                 | 2     | 70        |     | [Grund]{.smallcaps} <br> '[Reason]{.smallcaps}'         | 7     | 57        |

::: {.callout-note collapse="true"}
## Orientation

Some families are [Cause]{.smallcaps}-oriented, others are [Effect]{.smallcaps}-oriented. Compare:

> *Emission$_C$ is a cause of pollution$_E$.*  
> *Pollution$_E$ is a consequence of emission$_C$.*

Orientation affects projection – [Cause]{.smallcaps}-oriented indicators generally project [Cause]{.smallcaps} on the subject, [Effect]{.smallcaps}-oriented indicators do the same with [Effect]{.smallcaps}. Modified indicator forms affect the projection according to their orientation.
...
| ------------------------------------------------------- | -------- | ---------------- | -------------------------------------------------------------------------- |
| [Ursache]{.smallcaps} <br> '[Cause]{.smallcaps}' | $+$      | $[0.5-1]$        | [Cause]{.smallcaps}-oriented prototype                                    |
| [Folge]{.smallcaps} <br> '[Consequence]{.smallcaps}' | $+$      | $1$              | [Effect]{.smallcaps}-oriented                                              |
...
| [Reduzieren]{.smallcaps}  <br>  '[Reduce]{.smallcaps}' | $−$      | $[0.5-0.75]$     | Prototypical distributional negative                                       |
| [Wirkung]{.smallcaps}  <br>  '[Effect]{.smallcaps}' | $\pm$    | $[0-1]$          | Widest range of influence (*Gegenwirkung* 'counter-effect', *wirkungslos* 'ineffective' ) |
: Table Caption {tbl-colwidths="[30, 15, 25, 40]"}
...
> object negation on [Cause]{.smallcaps} (*Verlust* 'loss') inverts polarity: $I = -1$.
...
<!-- - For the theoretical motivation behind polarity and salience, see [Framework](../framework/framework.qmd) -->
- For how annotations are transformed into $(C, E, I)$ tuples, see [Tuple Construction](../processing/tuple-construction.qmd)
- For the C-BERT model trained on this data, see [C-BERT](c-bert.qmd)
